...
def exec_query(name, region, Sub_region, page,ings,not_ings, category, not_category, continent, process, utensil, recipe_ids,include_nutrBorders=None, dict_nut_boundaries={}):
	...
	queries = [
		...
		"select Distinct(recipes2.Recipe_id) from recipes2 where recipe_id not in (select recipe_id from ingredients where ingredient_name like \"%{}%\"){}",
...
def search_recipeInfo(id):
	...
	if row[0]['Source']=='AllRecipes':
		...
		row[0]['Source']='Genius Kitchen(food.com)'
	x=row[0]['url'].split('/')
	# print(x)
	if x[0]=="http:":
		x[0]='https:'
